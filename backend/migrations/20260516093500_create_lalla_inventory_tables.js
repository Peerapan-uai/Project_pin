...
exports.up = async (knex) => {
  ...
  await knex.schema.createTable('part_requests', (t) => {
    ...
    t.enu('status', ['pending', 'approved', 'rejected', 'returned'])
      .notNullable()
      .defaultTo('pending')
    ...
    t.foreign('tech_id').references('user_id').inTable('users').onDelete('CASCADE')

    t.foreign('part_id').references('part_id').inTable('spare_parts').onDelete('CASCADE')

    t.foreign('approved_by').references('user_id').inTable('users').onDelete('SET NULL')
  })
  ...
  await knex.schema.createTable('repair_proposals', (t) => {
    ...
    t.enu('status', ['pending', 'approved_repair', 'approved_replace', 'rejected'])
      .defaultTo('pending')
      .notNullable()
    ...
    t.foreign('reviewed_by').references('user_id').inTable('users').onDelete('SET NULL')
  })
}
